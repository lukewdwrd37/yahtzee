...
var hold = function(diToHold, holdno) {
    ...
    if (!diceJar[diToHold].rollable){
      document.getElementById(holdno).innerHTML = "[O]";
    }
    else{
...
release_the_tallyman = function(){
  ...
  console.log(tallymans_tallyboard);
}
  ...
  switch (scorename){
    ...
    case 'sm_straight':
          ...
          for(var di = 0; di<ordered_dice.length; di++){
            ...
            else if(ordered_dice == 1,2,3,4 || 2,3,4,5 || 3,4,5,6) {
              passes_test = true
              break
            }
            else{
    ...
    case '4_ofaKind':
      var diceOrder = release_the_tallyman();
      for(var i in diceOrder) {
        if (diceOrder[i] == 4) {
          current_score += diceOrder[i]*[i];
          document.getElementById("score").innerHTML = current_score;
        }
      }
      break;
...
// Replicating the method that adds the amount of each dice for 'single points'
